...
const OUTPUT = new URL('../../data/research/stock-near-high-kd-global-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_NEAR_HIGH_KD_GLOBAL_V1.md', import.meta.url);
...
function configs() {
  const rows = [];
  for (const maxStochastic of [70]) {
    for (const minNearYearHigh of [0.9]) {
      for (const maxReturn5 of [999]) {
        for (const holdDays of [20]) {
          rows.push({
            id: `near_high_kd_k${maxStochastic}_nyh${minNearYearHigh}_r5${maxReturn5}_h${holdDays}`,
            minTradeValue: 40e6,
            minScore: 45,
            maxStochastic,
            maxRsi: 85,
...
async function main() {
  const identityInput = {
    strategyId: 'stock_near_high_kd_global_v1',
    dataSources: ['tw_backtest_candidate_features', 'stochastic_rsi_features', 'daily_ohlcv'],
    setupRules: ['純個股近年高附近', 'KD 小於 70', '全球環境為正', '排除弱量'],
    triggerRules: ['強勢股冷卻後仍維持近高區，隔日進場'],
  ...
  await fs.writeFile(OUTPUT, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
  await fs.writeFile(REPORT, `# 純個股近高 KD 全球順風 v1\n\n- 驗證期：${output.validation.startMonth}~${output.validation.endMonth}\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark.averageMonthlyReturnPct}%\n- 是否達月均 5%：${passed}\n- 結論：${output.conclusion}\n\n此策略只交易個股，不以 ETF/0050 為主；核心是 nearYearHigh>=0.9、KD<70、全球環境為正、排除弱量，固定 20 日持有。\n`, 'utf8');
  await appendExperiment({ ...identityInput, parameters: { configsTested: allConfigs.length }, trainPeriod: { months: 60 }, validationPeriod: { months: 18, stepMonths: 18 }, costModel: { roundTripPct: COST_PCT }, executionModel: { entry: 'candidate_next_entry', exit: 'fixed 20-day hold with protective stop', settlement: 'T+2 cash delay' }, metrics, resultStatus: passed ? 'passed' : 'failed', failureReason: passed ? null : output.conclusion, passedMinimum: passed, passedHighProfit: false, allowRetest: false, notes: '純個股近高 KD 全球順風策略，不以 ETF 或 0050 為交易主體。' });
