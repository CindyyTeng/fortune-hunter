...
const OUTPUT = new URL('../../data/research/stock-reclaim-reversal-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_RECLAIM_REVERSAL_V1.md', import.meta.url);
...
function pass(row, config) {
  ...
  if ((row.rsi14 || 999) > config.maxRsi) return false;
  if (!row.falseBreakdownReclaim && !row.supportBounce && !row.crossAboveMa20) return false;
  ...
  if (config.excludeDistribution && row.highVolumeDistribution) return false;
  if (config.requireMa20Rising && !row.ma20Rising) return false;
  return true;
}

function rank(row, mode) {
  const reclaim = (row.falseBreakdownReclaim ? 30 : 0) + (row.supportBounce ? 20 : 0) + (row.crossAboveMa20 ? 15 : 0);
  const power = (row.signalScore || 0) + reclaim + (row.nearYearHigh || 0) * 20 - Math.max(0, row.return5Pct || 0);
  const risk = (row.atr14Pct || 0) * 2 + Math.max(0, row.gapUpPct || 0) + Math.max(0, row.distanceToMa20Pct || 0);
  if (mode === 'reclaim_quality') return power - risk;
...
function configs() {
  const rows = [];
  for (const minNearYearHigh of [0.65, 0.75, 0.85]) {
    for (const maxReturn5 of [0, 4, 999]) {
      for (const holdDays of [5, 8, 12]) {
          rows.push({
            id: `reclaim_nyh${minNearYearHigh}_r5${maxReturn5}_h${holdDays}`,
            minTradeValue: 40e6,
            minScore: 45,
            maxRsi: 85,
            minNearYearHigh,
            ...
            holdDays,
            stopLossPct: holdDays <= 5 ? 5 : 7,
            takeProfitPct: holdDays <= 5 ? 8 : 13,
            trailTriggerPct: holdDays <= 5 ? 6 : 10,
            trailGivebackPct: 5,
            rankMode: 'reclaim_quality',
            maxEntriesPerDay: 10,
            maxOpen: 20,
            positionPct: 8,
            accountRiskPct: 0.8,
...
async function main() {
  const identityInput = {
    strategyId: 'stock_reclaim_reversal_v1',
    dataSources: ['tw_backtest_candidate_features', 'support_reclaim_features', 'daily_ohlcv'],
    setupRules: ['純個股假跌破收復', '支撐反彈', '重新站回 MA20', '排除高量出貨'],
    triggerRules: ['收復或支撐反彈後隔日進場'],
