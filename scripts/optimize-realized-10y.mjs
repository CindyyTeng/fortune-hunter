...
const STOCK_FIXED_OOS_V9_OUTPUT = new URL(
  '../data/research/stock-fixed-top5-oos-v9.json',
  import.meta.url
);
const STOCK_FIXED_OOS_V10_OUTPUT = new URL(
  '../data/research/stock-fixed-top5-oos-v10.json',
  import.meta.url
);
const STOCK_FIXED_OOS_V11_OUTPUT = new URL(
...
function fixedTopFiveOosV10Configs(base) {
  const rows = [];
  const trailOptions = [
    null,
    { triggerPct: 8, givebackPct: 5, lockPct: 2 },
    { triggerPct: 10, givebackPct: 6, lockPct: 2 }
  ];
  for (const maxOpenPositions of [12]) {
    const standardPct = 8;
    for (const holdDays of [15, 20]) {
      for (const maxEntriesPerDay of [3, 5]) {
        for (const monthlyEquityBrakePct of [-4, -6]) {
                    ...
                    for (const rankMode of ['score', 'riskAdjustedMomentum']) {
                      for (const trail of trailOptions) {
                        rows.push({
                          ...
                          collectTrades: false,
                          researchVariant: 'stock_fixed_top5_oos_v10'
                        });
                      }
                    }
                  }
...
`, 'utf8');
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v9')) {
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const configs = fixedTopFiveOosV8Configs(base);
    ...
    if (!selected) throw new Error('fixed top5 v9 找不到符合訓練門檻的組合');
    const validation = simulateRange(
      daysFor(selected.config),
      selected.config,
      marketRegimes,
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v9',
      universe: '純個股，不使用 ETF 或 0050 作為交易標的；0050 僅作為比較基準',
      selection: {
        trainPeriod: '2016-08～2021-12',
        rule: '沿用 v8 參數集合與完整 simulateRange；訓練期先要求至少 300 筆且最大回撤不低於 -20%，再優先選訓練月均報酬最高組合',
        testedConfigs: configs.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(STOCK_FIXED_OOS_V9_OUTPUT, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    console.log(JSON.stringify({
      output: STOCK_FIXED_OOS_V9_OUTPUT.pathname,
      selection: output.selection,
      validation: output.validation,
      metrics,
      benchmark0050,
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v10')) {
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    ...
    const daysFor = config => daysByExitRule.get(JSON.stringify(config.exitRule));
    const trainRows = configs.map(config => simulateRange(daysFor(config), config, marketRegimes, '2016-08', '2021-12'));
    const selected = trainRows.filter(result => (
      result.trades >= 300
      && result.maxDrawdownPct >= -20
      && result.config.monthlyEquityBrakePct !== null
    ...
    if (!selected) throw new Error('fixed top5 v10 找不到符合訓練門檻的組合');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v10',
      universe: '純個股，不使用 ETF 或 0050 作為交易標的；0050 僅作為比較基準',
      selection: {
        trainPeriod: '2016-08～2021-12',
        rule: '沿用完整 simulateRange；以 v7/v9 附近參數為核心，加入 trailing stop 變體；訓練期回撤需不低於 -20%',
        testedConfigs: configs.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(STOCK_FIXED_OOS_V10_OUTPUT, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    console.log(JSON.stringify({ output: STOCK_FIXED_OOS_V10_OUTPUT.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v11')) {
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const configs = fixedTopFiveOosV10Configs(base);
    ...
    if (!selected) throw new Error('fixed top5 v11 找不到符合訓練門檻的組合');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v11',
      universe: '純個股，不使用 ETF 或 0050 作為交易標的；0050 僅作為比較基準',
      selection: {
        trainPeriod: '2016-08～2021-12',
        rule: '沿用 v10 參數集合與完整 simulateRange；訓練期放寬最大回撤到 -25%，再優先選訓練月均報酬最高組合',
        testedConfigs: configs.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v18')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v18.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V18.md', import.meta.url);
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    ...
    if (!baseSelected) throw new Error('fixed top5 v18 no qualified base candidate');
    const variants = [];
    for (const minMarketMom20Pct of [-8, -5, -2, 0]) {
      for (const maxMarketVol20Pct of [20, 25, 30, 100]) {
        for (const targetMarketVolPct of [12, 16, 20]) {
          for (const weakMarketPositionMultiplier of [0.5, 0.7, 0.85]) {
            for (const strongMarketPositionMultiplier of [1, 1.15, 1.3]) {
              variants.push({
                ...baseSelected.config,
                minMarketMom20Pct,
                maxMarketVol20Pct,
    ...
    const rows = variants.map(config => {
      ...
      return {
        ...
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        stabilityScore: fullTrain.full.average * 1.35
          + worstSegmentAveragePct
          + Math.min(quality.profitFactor, 2.5)
          + fullTrain.maxDrawdownPct * 0.04
          + Math.min(fullTrain.trades, 600) / 600
      };
    });
    const selected = rows.filter(row => (
      row.trades >= 300
      && row.maxDrawdownPct >= -18
      && row.profitFactor >= 1.2
      && row.worstSegmentAveragePct >= -0.5
      && row.worstSegmentDrawdownPct >= -18
    )).sort((a, b) => b.stabilityScore - a.stabilityScore || b.full.average - a.full.average)[0];
    if (!selected) throw new Error('fixed top5 v18 no qualified train variant');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const targetMet = metrics.averageMonthlyReturnPct >= 5 && metrics.maximumDrawdownPct >= -20 && metrics.trades >= 300 && metrics.beats0050;
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v18',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '保留 v15 子集合，測大盤動能、波動與強弱盤部位倍率。',
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v19')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v19.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V19.md', import.meta.url);
    ...
    const scoreTrain = row => row.full.average * 1.8
      + row.worstSegmentAveragePct * 1.2
      + Math.min(row.profitFactor, 2.6)
      + row.maxDrawdownPct * 0.09
      + row.worstSegmentDrawdownPct * 0.04
      + Math.min(row.trades, 650) / 120
      - Math.max(0, 350 - row.trades) / 70;
    const baseRows = seedConfigs.map(config => {
      const filtered = withSubset(config);
      const fullTrain = simulateRange(daysFor(filtered), filtered, marketRegimes, '2016-08', '2021-12', true, true);
      const quality = tradeQuality(fullTrain);
    ...
    }).sort((a, b) => scoreTrain(b) - scoreTrain(a));
    ...
    const variants = [];
    for (const holdDays of [15, 20]) {
      for (const stopLoss of [5, 7]) {
        for (const monthlyEquityBrakePct of [-4, -5]) {
          for (const accountRiskPct of [0.75, 1]) {
            for (const minMarketMom20Pct of [-5, -2]) {
              for (const maxEntriesPerDay of [3, 5]) {
                for (const targetMarketVolPct of [16, 20]) {
                  variants.push({
                    ...baseSelected.config,
                    ...
                    strongMarketMom5Pct: 0,
                    strongMarketMom20Pct: 0,
                    strongMarketPositionMultiplier: 1,
                    weakMarketPositionMultiplier: 0.75,
                    exitRule: {
                      ...baseSelected.config.exitRule,
                      holdDays,
                      overrideStopLossPct: stopLoss,
                      stopMode: 'close'
                    }
                  });
                }
    ...
    const selected = rows.find(row => (
      row.trades >= 300
      && row.maxDrawdownPct >= -16
      && row.profitFactor >= 1.25
      && row.worstSegmentAveragePct >= -0.15
      && row.worstSegmentDrawdownPct >= -15
    )) || rows[0];
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v19',
      universe: '純個股，不以 ETF 或 0050 為主要交易標的。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '以 v15 子集合為底，訓練期同時排序月均、回撤與交易數，避免 v18 只增加交易卻放大回撤。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 純個股 Top5 OOS v19\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark0050.averageMonthlyReturnPct}%\n- 對 v15：月均改善=${metrics.improvesMonthlyVsV15}，回撤改善=${metrics.improvesDrawdownVsV15}，交易數改善=${metrics.improvesTradesVsV15}\n- 結論：${output.conclusion}\n\nv19 保留純個股核心，不使用 ETF/0050 當主要交易標的。這版只用 2016-08~2021-12 訓練期挑參數，再凍結到 2022-01~2026-05 驗證，避免未來資料洩漏。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v20')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v20.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V20.md', import.meta.url);
    const v15 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url), 'utf8'));
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    ...
    const scoreTrain = row => row.full.average * 1.4
      + row.worstSegmentAveragePct * 1.4
      + Math.min(row.profitFactor, 2.6)
      + row.maxDrawdownPct * 0.16
      + row.worstSegmentDrawdownPct * 0.08
      + Math.min(row.trades, 600) / 120
      - Math.max(0, 350 - row.trades) / 55;
    const baseRows = seedConfigs.map(config => {
      const filtered = withSubset(config);
      const fullTrain = simulateRange(daysFor(filtered), filtered, marketRegimes, '2016-08', '2021-12', true, true);
      const quality = tradeQuality(fullTrain);
    ...
    const baseSelected = baseRows.find(row => (
      ...
      && row.maxDrawdownPct >= -18
      && row.profitFactor >= 1.2
      && row.worstSegmentAveragePct >= -0.2
      && row.worstSegmentDrawdownPct >= -16
    )) || baseRows[0];
    const variants = [];
    for (const standardPct of [5, 6, 7]) {
      for (const maxOpenPositions of [8, 10, 12]) {
        for (const maxEntriesPerDay of [2, 3]) {
          for (const accountRiskPct of [0.5, 0.75]) {
            for (const targetMarketVolPct of [10, 12, 14]) {
              for (const monthlyEquityBrakePct of [-3, -4]) {
                for (const stopLoss of [4, 5]) {
                  variants.push({
                    ...baseSelected.config,
                    standardPct,
                    defensivePct: standardPct,
                    ...
                    strongMarketMom20Pct: 0,
                    strongMarketPositionMultiplier: 1,
                    weakMarketPositionMultiplier: 0.7,
                    exitRule: {
                      ...baseSelected.config.exitRule,
                      holdDays: 20,
                      overrideStopLossPct: stopLoss,
    ...
    const selected = rows.find(row => (
      row.trades >= 300
      && row.maxDrawdownPct >= -12
      && row.profitFactor >= 1.25
      && row.worstSegmentAveragePct >= -0.1
      && row.worstSegmentDrawdownPct >= -12
    )) || rows[0];
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v20',
      universe: '純個股，不以 ETF 或 0050 為主要交易標的。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '以 v19 高交易數方向為底，改測低曝險、低波動目標與較嚴回撤條件。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v22')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v22.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V22.md', import.meta.url);
    const v15 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url), 'utf8'));
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    ...
    const baseRows = seedConfigs.map(config => {
      ...
      const worstSegmentAveragePct = Math.min(...segments.map(row => row.full.average));
      const worstSegmentDrawdownPct = Math.min(...segments.map(row => row.maxDrawdownPct));
      return { ...fullTrain, config: filtered, profitFactor: quality.profitFactor, worstSegmentAveragePct, worstSegmentDrawdownPct };
    }).sort((a, b) => b.full.average - a.full.average);
    const baseSelected = baseRows.find(row => row.trades >= 250 && row.maxDrawdownPct >= -22 && row.profitFactor >= 1.15) || baseRows[0];
    const variants = [];
    for (const accountRiskPct of [0.75, 1, 1.15]) {
      for (const maxEntriesPerDay of [3, 4, 5]) {
        for (const monthlyEquityBrakePct of [-4, -5]) {
          for (const targetMarketVolPct of [16, 18, 20]) {
            for (const weakMarketPositionMultiplier of [0.7, 0.8, 0.9]) {
              variants.push({
                ...baseSelected.config,
                accountRiskPct,
                starterRiskPct: accountRiskPct,
    ...
    const rows = variants.map(config => {
      ...
      return {
        ...
        score: fullTrain.full.average * 2
          + Math.min(quality.profitFactor, 2.5)
          + fullTrain.maxDrawdownPct * 0.12
          + Math.min(fullTrain.trades, 650) / 140
          + worstSegmentAveragePct
      };
    }).sort((a, b) => b.score - a.score);
    const selected = rows.find(row => row.trades >= 300
      && row.maxDrawdownPct >= -18
      && row.profitFactor >= 1.25
      && row.worstSegmentAveragePct >= -0.2
      && row.worstSegmentDrawdownPct >= -16) || rows[0];
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v22',
      universe: '純個股，不以 ETF 或 0050 為主要交易標的。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '在 v19 附近修正回撤，測弱盤部位倍率、單日進場數、月損煞車與目標波動。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 純個股 Top5 OOS v22\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark0050.averageMonthlyReturnPct}%\n- 對 v15：月均改善=${metrics.improvesMonthlyVsV15}，回撤改善=${metrics.improvesDrawdownVsV15}，交易數改善=${metrics.improvesTradesVsV15}\n- 結論：${output.conclusion}\n\nv22 保留純個股核心，不使用 ETF/0050 當主要交易標的。這版在 v19 附近修正回撤，仍只用 2016-08~2021-12 訓練期挑參數，再凍結到 2022-01~2026-05 驗證。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v23')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v23.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V23.md', import.meta.url);
    const v15 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url), 'utf8'));
    const v19 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v19.json', import.meta.url), 'utf8'));
    const v22 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v22.json', import.meta.url), 'utf8'));
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const withSubset = config => ({
    ...
    const seedConfigs = fixedTopFiveOosV10Configs(base).map(withSubset);
    const daysByExitRule = new Map();
    const daysFor = config => {
      const key = JSON.stringify(config.exitRule);
      if (!daysByExitRule.has(key)) daysByExitRule.set(key, broadDaysForExitRule(config.exitRule));
    ...
    const seedRows = seedConfigs.map(config => {
      const result = simulateRange(daysFor(config), config, marketRegimes, '2016-08', '2021-12', true, true);
      const quality = tradeQuality(result);
      return { ...result, config, profitFactor: quality.profitFactor };
    }).sort((a, b) => b.full.average - a.full.average || b.profitFactor - a.profitFactor);
    const seeds = seedRows.filter(row => row.trades >= 250 && row.maxDrawdownPct >= -22 && row.profitFactor >= 1.15).slice(0, 3).map(row => row.config);
    const variants = [];
    for (const seed of seeds) {
      for (const accountRiskPct of [0.5, 0.75]) {
        for (const maxEntriesPerDay of [2, 3]) {
          for (const monthlyEquityBrakePct of [-3, -4, -5]) {
            for (const targetMarketVolPct of [14, 16, 18]) {
              for (const weakMarketPositionMultiplier of [0.5, 0.65, 0.75]) {
                for (const overrideStopLossPct of [5]) {
                  for (const holdDays of [20]) {
                    variants.push({
                      ...seed,
                      accountRiskPct,
                      starterRiskPct: accountRiskPct,
    ...
    const rows = variants.map(config => {
      ...
      const segments = trainSegments.map(segment => {
        const result = simulateRange(daysFor(config), config, marketRegimes, segment.start, segment.end, true, true);
        return {
          ...segment,
          averageMonthlyReturnPct: result.full.average,
          maximumDrawdownPct: result.maxDrawdownPct,
          trades: result.trades
        };
      });
      const worstSegmentAveragePct = Math.min(...segments.map(row => row.averageMonthlyReturnPct));
      const worstSegmentDrawdownPct = Math.min(...segments.map(row => row.maximumDrawdownPct));
      return {
        ...fullTrain,
        config,
        profitFactor: quality.profitFactor,
        ...
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        score: fullTrain.full.average * 2.4
          + Math.min(quality.profitFactor, 2.5) * 1.2
          + Math.min(fullTrain.trades, 700) / 160
          + worstSegmentAveragePct * 1.6
          + fullTrain.maxDrawdownPct * 0.16
          + worstSegmentDrawdownPct * 0.06
      };
    }).sort((a, b) => b.score - a.score);
    const selected = rows.find(row => (
      row.trades >= 300
      && row.maxDrawdownPct >= -14
      && row.profitFactor >= 1.3
      && row.worstSegmentAveragePct >= 0
      && row.worstSegmentDrawdownPct >= -13
    )) || rows.find(row => (
      row.trades >= 300
      && row.maxDrawdownPct >= -16
      && row.profitFactor >= 1.25
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v23',
      universe: '純個股，不以 ETF 或 0050 作為主要交易標的',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '以 v15/v19/v22 已驗證參數為種子，只用訓練期挑選低回撤、高交易數、三段訓練期較穩定的固定參數；validation 參數凍結。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
        trainTrades: selected.trades,
        trainProfitFactor: selected.profitFactor,
        trainSegments: selected.segments,
        worstSegmentAveragePct: round(selected.worstSegmentAveragePct),
        worstSegmentDrawdownPct: selected.worstSegmentDrawdownPct
      },
      validation: { period: '2022-01~2026-05', months: validation.full.months, parametersFrozen: true },
      ...
      liveTradingReady: false,
      conclusion: targetMet
        ? '達到月均 5% 初步門檻，但仍需更長紙上交易驗證後才可考慮實盤。'
        : `未達月均 5% 可實盤門檻；validation 月均 ${metrics.averageMonthlyReturnPct}%，不可 paper trading、不可實盤。`
    };
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 純個股 Top5 OOS v23\n\n- 驗證期：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark0050.averageMonthlyReturnPct}%\n- 是否達月均 5%：${targetMet}\n- 結論：${output.conclusion}\n\nv23 使用純個股交易，不以 ETF/0050 為主。此版以 v15/v19/v22 為種子，試圖在不偷看 validation 的前提下降低 v19 的回撤，同時維持交易數與月均報酬。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v24')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v24.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V24.md', import.meta.url);
    const v15 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url), 'utf8'));
    const v19 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v19.json', import.meta.url), 'utf8'));
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    ...
    const seeds = fixedTopFiveOosV10Configs(base).map(withSubset).map(config => {
      const fullTrain = simulateRange(daysFor(config), config, marketRegimes, '2016-08', '2021-12', true, true);
      const quality = tradeQuality(fullTrain);
      const segments = trainSegments.map(segment => simulateRange(daysFor(config), config, marketRegimes, segment.start, segment.end, true, true));
      return {
        ...
        config,
        profitFactor: quality.profitFactor,
        worstSegmentAveragePct: Math.min(...segments.map(row => row.full.average)),
        worstSegmentDrawdownPct: Math.min(...segments.map(row => row.maxDrawdownPct))
      };
    }).filter(row => row.trades >= 300 && row.maxDrawdownPct >= -20 && row.profitFactor >= 1.25)
      .sort((a, b) => b.full.average - a.full.average || b.trades - a.trades)
      .slice(0, 2)
      .map(row => row.config);
    const variants = [];
    for (const seed of seeds) {
      for (const accountRiskPct of [0.75, 1]) {
        for (const maxEntriesPerDay of [4, 5]) {
          for (const monthlyEquityBrakePct of [-4, -5]) {
            for (const targetMarketVolPct of [18, 20]) {
              for (const weakMarketPositionMultiplier of [0.75, 1]) {
                for (const overrideStopLossPct of [7]) {
                  variants.push({
                    ...seed,
                    accountRiskPct,
                    starterRiskPct: accountRiskPct,
    ...
    const rows = variants.map(config => {
      ...
      const segments = trainSegments.map(segment => {
        const result = simulateRange(daysFor(config), config, marketRegimes, segment.start, segment.end, true, true);
        return { id: segment.id, start: segment.start, end: segment.end, averageMonthlyReturnPct: result.full.average, maximumDrawdownPct: result.maxDrawdownPct, trades: result.trades };
      });
      const worstSegmentAveragePct = Math.min(...segments.map(row => row.averageMonthlyReturnPct));
      const worstSegmentDrawdownPct = Math.min(...segments.map(row => row.maximumDrawdownPct));
      return {
        ...
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        score: fullTrain.full.average * 2.6
          + worstSegmentAveragePct
          + Math.min(quality.profitFactor, 2.3)
          + Math.min(fullTrain.trades, 650) / 150
          + fullTrain.maxDrawdownPct * 0.08
      };
    }).sort((a, b) => b.score - a.score);
    const selected = rows.find(row => (
      row.trades >= 450
      && row.maxDrawdownPct >= -18
      && row.profitFactor >= 1.45
      && row.worstSegmentAveragePct >= 0.3
      && row.worstSegmentDrawdownPct >= -16
    )) || rows.find(row => row.trades >= 350 && row.maxDrawdownPct >= -20 && row.profitFactor >= 1.3) || rows[0];
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v24',
      universe: '純個股，不以 ETF 或 0050 作為主要交易標的',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '以完整純個股 Top5 候選條件重新找種子，選擇訓練期高月均、足夠交易數且分段不過度失衡的攻守折衷參數；validation 參數凍結。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
      ...
      metrics,
      benchmark0050,
      baselines: { v15: v15.metrics, v19: v19.metrics },
      targetMonthlyReturnPct: 5,
      targetMet,
      paperTradingReady: false,
      liveTradingReady: false,
  ...
  if (process.argv.includes('--stock-fixed-top5-oos-v17')) {
    ...
    if (!baseSelected) throw new Error('fixed top5 v17 no qualified base candidate');
    const variants = [];
    for (const profitLockPct of [null, 8, 10, 12]) {
      for (const monthPeakTriggerPct of [null, 3, 5]) {
        for (const monthGivebackPct of [1.5, 2, 3]) {
          for (const lossBrakePct of [null, -3, -4]) {
            for (const positiveMonthPositionMultiplier of [1, 1.15]) {
              variants.push({
                ...baseSelected.config,
                profitLockPct,
                profitLockAction: profitLockPct === null ? 'block' : 'exit_next_open',
                monthPeakTriggerPct,
                monthGivebackPct,
                monthDrawdownAction: monthPeakTriggerPct === null ? 'block' : 'exit_next_open',
                lossBrakePct,
                lossBrakeAction: lossBrakePct === null ? 'block' : 'exit_next_open',
                monthPerformancePositioning: positiveMonthPositionMultiplier !== 1,
                monthPerformanceTriggerPct: 2,
                positiveMonthPositionMultiplier,
                negativeMonthPositionMultiplier: 0.75,
                positiveMonthRiskMultiplier: positiveMonthPositionMultiplier,
                negativeMonthRiskMultiplier: 0.75
              });
            }
          }
        }
    ...
    const rows = variants.map(config => {
      ...
      return {
        ...
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        stabilityScore: fullTrain.full.average * 1.25 + worstSegmentAveragePct + Math.min(quality.profitFactor, 2.5) + fullTrain.maxDrawdownPct * 0.04 + Math.min(fullTrain.trades, 600) / 600
      };
    });
    const selected = rows.filter(row => (
      row.trades >= 280
    ...
    if (!selected) throw new Error('fixed top5 v17 no qualified train variant');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v17',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '保留 v15 子集合與完整 config，測獲利鎖定、月內回吐、虧損煞車與正月加碼。',
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 固定 Top5 純個股 OOS v17\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 同期月均：${benchmark0050.averageMonthlyReturnPct}%\n- 結論：${output.conclusion}\n\nv17 保留 v15 的正向子集合，測月內獲利保護與動態部位。不使用 ETF/0050 作為交易標的。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v16')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v16.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V16.md', import.meta.url);
    const v15 = JSON.parse(await fs.readFile(new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url), 'utf8'));
    const seedConfigs = [{ ...v15.selection.selectedConfig }];
    const withSubset = config => ({
      ...config,
      maxAtr14Pct: Math.min(config.maxAtr14Pct ?? 100, 6),
      maxGap: Math.min(config.maxGap ?? 100, 8),
    ...
    const variants = seedConfigs.flatMap(config => {
      const filtered = withSubset(config);
      return [6, 8, 10].flatMap(maxOpenPositions => (
        [2, 3, 4].flatMap(maxEntriesPerDay => (
          [8, 10, 12].flatMap(positionPct => (
            [0.8, 1, 1.25].flatMap(accountRiskPct => (
              [-4, -5].flatMap(monthlyEquityBrakePct => (
                [12, 16, 20].flatMap(targetMarketVolPct => (
                  [10, 15, 20].flatMap(holdDays => (
                    [5, 7].map(stopLoss => ({
                      ...filtered,
                      maxOpenPositions,
                      maxEntriesPerDay,
    ...
    const rows = variants.map(config => {
      ...
      return {
        ...
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        stabilityScore: fullTrain.full.average * 1.5
          + worstSegmentAveragePct * 1.1
          + Math.min(quality.profitFactor, 2.5)
          + fullTrain.maxDrawdownPct * 0.045
          + Math.min(fullTrain.trades, 650) / 650
      };
    });
    const qualified = rows.filter(row => (
      ...
      && row.worstSegmentAveragePct >= -0.5
      && row.worstSegmentDrawdownPct >= -20
    )).sort((a, b) => b.stabilityScore - a.stabilityScore || b.full.average - a.full.average);
    const selected = qualified[0] || rows.sort((a, b) => b.stabilityScore - a.stabilityScore || b.full.average - a.full.average)[0];
    if (!selected) throw new Error('fixed top5 v16 no train variant');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v16',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '保留 v15 子集合，搜尋更集中持股、進場數、部位風險與出場天數。',
        trainQualified: qualified.length > 0,
        testedConfigs: variants.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 固定 Top5 純個股 OOS v16\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 同期月均：${benchmark0050.averageMonthlyReturnPct}%\n- 結論：${output.conclusion}\n\nv16 保留 v15 子集合，測更集中持股與風險配置。不使用 ETF/0050 作為交易標的。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v15')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v15.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V15.md', import.meta.url);
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const seedConfigs = fixedTopFiveOosV10Configs(base);
    ...
    const variants = [];
    for (const holdDays of [10, 15, 20]) {
      for (const stopLoss of [5, 7, 9]) {
        for (const monthlyEquityBrakePct of [-3, -4, -5, -6]) {
          for (const targetMarketVolPct of [12, 16, 20]) {
            for (const maxEntriesPerDay of [3, 5]) {
              for (const accountRiskPct of [1, 1.25]) {
                variants.push({
                  ...baseSelected.config,
                  monthlyEquityBrakePct,
                  targetMarketVolPct,
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v15',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 固定 Top5 純個股 OOS v15\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 同期月均：${benchmark0050.averageMonthlyReturnPct}%\n- 結論：${output.conclusion}\n\nv15 保留 v14 子集合條件，改測較小範圍的出場與風控變體。不使用 ETF/0050 作為交易標的。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v14')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v14.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V14.md', import.meta.url);
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const configs = fixedTopFiveOosV10Configs(base);
    ...
    const rows = configs.map(config => {
      ...
      const fullTrain = simulateRange(daysFor(config), filteredConfig, marketRegimes, '2016-08', '2021-12', true, true);
      const quality = tradeQuality(fullTrain);
      const segments = trainSegments.map(segment => simulateRange(daysFor(config), filteredConfig, marketRegimes, segment.start, segment.end, true, true));
      const worstSegmentAveragePct = Math.min(...segments.map(row => row.full.average));
      const worstSegmentDrawdownPct = Math.min(...segments.map(row => row.maxDrawdownPct));
      return {
        ...fullTrain,
    ...
    if (!selected) throw new Error('fixed top5 v14 no qualified train candidate');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v14',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '沿用 Top5 完整 portfolio 回測，加入訓練期交易明細挑出的 ATR、跳空、近年高、MA20 乖離與大盤動能子集合條件。',
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
        trainTrades: selected.trades,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 固定 Top5 純個股 OOS v14\n\n- 驗證期間：2022-01~2026-05\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 同期月均：${benchmark0050.averageMonthlyReturnPct}%\n- 結論：${output.conclusion}\n\nv14 把上一輪交易明細診斷條件套回完整 portfolio 回測，不使用 ETF/0050 作為交易標的。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
  if (process.argv.includes('--stock-fixed-top5-oos-v13')) {
    const outputFile = new URL('../data/research/stock-fixed-top5-oos-v13.json', import.meta.url);
    const reportFile = new URL('../docs/STOCK_FIXED_TOP5_OOS_V13.md', import.meta.url);
    const search = JSON.parse(await fs.readFile(OUTPUT, 'utf8'));
    const base = { ...search.bestBalanced.config };
    delete base.collectTrades;
    const configs = fixedTopFiveOosV10Configs(base);
    ...
    const trainRows = configs.map(config => {
      const fullTrain = simulateRange(daysFor(config), config, marketRegimes, '2016-08', '2021-12', true, true);
      const fullQuality = tradeQuality(fullTrain);
      ...
      const worstSegmentProfitFactor = Math.min(...segmentResults.map(row => row.profitFactor));
      const stabilityScore = fullTrain.full.average * 0.8
        + worstSegmentAveragePct * 1.2
        + Math.min(fullQuality.profitFactor, 2.5) * 0.55
        + Math.min(worstSegmentProfitFactor, 2) * 0.45
        + positiveSegments * 0.35
        + fullTrain.maxDrawdownPct * 0.035
        + Math.min(fullTrain.trades, 700) / 700 * 0.25;
      return {
        ...fullTrain,
        fullProfitFactor: fullQuality.profitFactor,
        trainSegments: segmentResults,
        positiveSegments,
        worstSegmentAveragePct,
        worstSegmentDrawdownPct,
        worstSegmentProfitFactor,
        stabilityScore
      };
    });
    const selected = trainRows.filter(result => (
      result.trades >= 300
      && result.maxDrawdownPct >= -22
      && result.fullProfitFactor >= 1.2
      && result.positiveSegments >= 3
      && result.worstSegmentAveragePct >= 0
      && result.worstSegmentDrawdownPct >= -18
      && result.worstSegmentProfitFactor >= 1
      && result.config.monthlyEquityBrakePct !== null
    )).sort((a, b) => (
      b.stabilityScore - a.stabilityScore
      || b.full.average - a.full.average
    ...
    if (!selected) throw new Error('fixed top5 v13 no qualified train candidate');
    const validation = simulateRange(daysFor(selected.config), selected.config, marketRegimes, '2022-01', '2026-05', true, true);
    const quality = tradeQuality(validation);
    const etfHistory = JSON.parse(await fs.readFile(ETF_HISTORY, 'utf8'));
    const benchmark0050 = benchmarkStats(etfHistory.series['0050.TW'] || [], '2022-01-01', '2026-05-31');
    ...
    const output = {
      generatedAt: new Date().toISOString(),
      strategyId: 'stock_fixed_top5_oos_v13',
      universe: '純個股候選；ETF 與 0050 僅作比較基準。',
      selection: {
        trainPeriod: '2016-08~2021-12',
        validationPeriod: '2022-01~2026-05',
        rule: '沿用 fixed-top5 v12 完整成交與風控模型，改用更嚴格分段穩定、PF 與回撤排序。',
        testedConfigs: configs.length,
        selectedConfig: riskConfig(selected.config),
        trainAverageMonthlyReturnPct: round(selected.full.average),
        trainMaximumDrawdownPct: selected.maxDrawdownPct,
        trainTrades: selected.trades,
        trainProfitFactor: selected.fullProfitFactor,
    ...
    await fs.writeFile(outputFile, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
    await fs.writeFile(reportFile, `# 固定 Top5 純個股 OOS v13\n\n- 驗證期間：2022-01~2026-05\n- 訓練選擇：分段穩定、PF、回撤優先\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 同期月均：${benchmark0050.averageMonthlyReturnPct}%\n- 結論：${output.conclusion}\n\n本策略仍以個股為主，排除 ETF/0050 作為交易標的。若未達標，不可接券商 API 實盤。\n`, 'utf8');
    console.log(JSON.stringify({ output: outputFile.pathname, report: reportFile.pathname, selection: output.selection, validation: output.validation, metrics, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
    return;
  }
