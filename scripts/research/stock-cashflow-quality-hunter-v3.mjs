...
const OUTPUT = new URL('../../data/research/stock-cashflow-quality-hunter-v3.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_CASHFLOW_QUALITY_HUNTER_V3.md', import.meta.url);
const STRATEGY_ID = 'stock_cashflow_quality_hunter_v3';
...
const setups = [
  {
    ...
    test: row => row.cashflow.operatingCashFlow > 0
      ...
      && row.cashflow.cashConversion >= 0.7
      && row.cashflow.accrualRatio <= 2
      && row.cashflow.debtRatioChangeYoY <= 5
  },
  {
    id: 'cashflow_acceleration',
    name: '營業現金流加速',
    test: row => row.cashflow.operatingCashFlow > 0
      && row.cashflow.operatingCashFlowYoY >= 10
      && row.cashflow.netIncome > 0
      && row.cashflow.accrualRatio <= 2
  },
  {
    ...
    test: row => row.cashflow.operatingCashFlow > 0
      ...
      && row.cashflow.debtRatio < 75
      && row.cashflow.assetGrowthYoY >= -20
      && row.cashflow.assetGrowthYoY <= 50
  },
  {
...
function configurations() {
  return setups.filter(setup => setup.id !== 'quality_momentum').flatMap(setup => [20, 40].flatMap(holdingDays => [10, 15].flatMap(topN => [8, 12].flatMap(stopDistancePct => ['positive_reaction', 'breakout_10d', 'ma20_restrengthen'].flatMap(trigger => ['risk_on'].map(marketMode => ({
...
function triggerVariants(history, eventIndex, marketByDate) {
  ...
  for (let index = eventIndex; index <= Math.min(eventIndex + 30, history.length - 2); index += 1) {
...
function candidate(row, variant, config, random = false) {
  return {
    ...
    positionPct: Math.min(12, 80 / config.topN),
    accountRiskPct: 0.7,
...
function run(context, map, config, startDate, endDate, suffix = '') {
  return simulateSignalMap(context, map, {
    ...
    accountRiskPct: 0.7,
    riskRules: {
      maxAccountRiskPct: 0.7,
      maxSinglePositionPct: 12,
      exposureLimits: {
        BULL_TREND: 85, BULL_PULLBACK: 70, RANGE_BOUND: 45,
        THEME_MOMENTUM: 85, HIGH_VOLATILITY: 20, BEAR_DEFENSE: 0
      },
      drawdownBlockPct: 8,
...
async function main() {
  ...
  const identityInput = {
    ...
    riskRules: { accountRiskPct: 0.7, maxPositionPct: 10, tPlusTwo: true },
  ...
  for (const fold of foldWindows(context.startDate, context.endDate, 54, 18)) {
    const trained = configs.map(config => {
      const result = run(context, signalMap(events, config), config, fold.trainStart, fold.trainEnd);
      const summary = result.summary;
      const pf = Number.isFinite(summary.profitFactor) ? Math.min(2.5, summary.profitFactor) : 0;
      return {
        config,
        result,
        score: summary.averageMonthlyEquityReturnPct * 12
          + pf * 2
          + summary.maximumDrawdownPct * 0.18
          + Math.min(200, summary.trades) * 0.01
          - summary.negativeMonths * 0.03
      };
    }).filter(row => (
      row.result.summary.trades >= 40
      && row.result.summary.maximumDrawdownPct >= -20
      && row.result.summary.averageMonthlyEquityReturnPct >= 0
      && row.result.summary.profitFactor >= 1
    ))
