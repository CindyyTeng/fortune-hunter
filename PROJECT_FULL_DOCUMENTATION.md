...
- Current default therefore keeps the original `30,000,000` traded-value floor, adds `std20 >= 2`, and leaves `TRADE_MODE_MAX_PLAN_RISK_PCT` at `99` unless explicitly overridden.
...
- Position cap: each simulated position uses at most `10%` of initial paper cash
