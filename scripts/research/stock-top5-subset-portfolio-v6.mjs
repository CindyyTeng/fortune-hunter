...
const OUTPUT = new URL('../../data/research/stock-top5-subset-portfolio-v6.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TOP5_SUBSET_PORTFOLIO_V6.md', import.meta.url);
...
async function main() {
  ...
  for (const filter of filters()) {
    ...
    for (const exposureScale of [1.47]) {
  ...
  const output = {
    ...
    selected: { filter: selected.filter, exposureScale: 1.47, train: summarize(trainTrades.filter(row => pass(row, selected.filter)), 1.47) },
    validation,
