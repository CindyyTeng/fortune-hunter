...
const OUTPUT = new URL('../../data/research/stock-target-upside-global-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TARGET_UPSIDE_GLOBAL_V1.md', import.meta.url);
...
function configs() {
  const rows = [];
  for (const minTargetFullRatio of [1.12, 1.15, 1.18]) {
    for (const minNearYearHigh of [0.75, 0.8, 0.85]) {
...
async function main() {
  const identityInput = {
    strategyId: 'stock_target_upside_global_v1',
    dataSources: ['tw_backtest_candidate_features', 'target_upside_features', 'global_composite_features'],
    setupRules: ['純個股目標空間大於 12%/15%/18%', '近年高相對位置不弱', '全球環境為正'],
    triggerRules: ['候選股隔日進場，固定 15/20 日驗證'],
  ...
  await fs.writeFile(OUTPUT, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
  await fs.writeFile(REPORT, `# 純個股目標空間全球順風 v1\n\n- 驗證期：${output.validation.startMonth}~${output.validation.endMonth}\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark.averageMonthlyReturnPct}%\n- 是否達月均 5%：${passed}\n- 結論：${output.conclusion}\n\n此策略只交易個股，不以 ETF/0050 為主；核心是 targetFull 目標空間、近高位置與全球順風。\n`, 'utf8');
  await appendExperiment({ ...identityInput, parameters: { configsTested: allConfigs.length }, trainPeriod: { months: 60 }, validationPeriod: { months: 18, stepMonths: 18 }, costModel: { roundTripPct: COST_PCT }, executionModel: { entry: 'candidate_next_entry', exit: 'fixed hold with protective stop', settlement: 'T+2 cash delay' }, metrics, resultStatus: passed ? 'passed' : 'failed', failureReason: passed ? null : output.conclusion, passedMinimum: passed, passedHighProfit: false, allowRetest: false, notes: '純個股目標空間全球順風策略，不以 ETF 或 0050 為交易主體。' });
