...
const OUTPUT = new URL('../../data/research/stock-top5-subset-portfolio-v7.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TOP5_SUBSET_PORTFOLIO_V7.md', import.meta.url);
...
async function main() {
  ...
  for (const filter of filters()) {
    ...
    for (const exposureScale of [1.5]) {
  ...
  const output = {
    ...
    strategyId: 'stock_top5_subset_portfolio_v7',
    source: 'stock-fixed-top5-oos-v13 trade details',
    selected: { filter: selected.filter, exposureScale: 1.5, train: summarize(trainTrades.filter(row => pass(row, selected.filter)), 1.5) },
    validation,
