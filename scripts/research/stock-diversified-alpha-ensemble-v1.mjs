...
const OUTPUT = new URL('../../data/research/stock-diversified-alpha-ensemble-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_DIVERSIFIED_ALPHA_ENSEMBLE_V1.md', import.meta.url);
const STRATEGY_ID = 'stock_diversified_alpha_ensemble_v1';
...
function addSignals(target, source, label, index) {
  for (const [date, rows] of source) {
    ...
    for (const candidate of rows) {
      list.push({
        ...
        positionPct: Math.min(10, candidate.positionPct || 10),
        accountRiskPct: 0.5,
...
function combinedSignals(context, revenue, shockReport, random = false) {
  const result = new Map();
  const index = futureBarsBySymbol(context);
  addSignals(result, revenueMap(revenue, random ? 'randomValidationSignals' : 'validationSignals'), '意外營收', index);
  const events = shockEvents(context);
  for (const fold of shockReport.folds.filter(row => row.status === '完成')) {
    addSignals(result, shockSignals(events, fold, random), '急跌止穩', index);
  }
...
function run(context, map, random = false) {
  return simulateSignalMap(context, map, {
    ...
    riskRules: {
      ...
      exposureLimits: { BULL_TREND: 80, THEME_MOMENTUM: 80, BULL_PULLBACK: 65, RANGE_BOUND: 45, HIGH_VOLATILITY: 20, BEAR_DEFENSE: 15 },
      drawdownBlockPct: 8,
...
async function main() {
  ...
  const result = run(context, combinedSignals(context, revenue, shockReport));
  const random = run(context, combinedSignals(context, revenue, shockReport, true), true);
  ...
  console.log(JSON.stringify({ validationPeriod: output.validationPeriod, metrics, fairRandom, benchmark0050, targetMet, conclusion: output.conclusion }, null, 2));
}
