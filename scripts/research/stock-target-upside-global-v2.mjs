...
const OUTPUT = new URL('../../data/research/stock-target-upside-global-v2.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TARGET_UPSIDE_GLOBAL_V2.md', import.meta.url);
...
function configs() {
  const rows = [];
  for (const minTargetFullRatio of [1.15, 1.18, 1.22]) {
    for (const minNearYearHigh of [0.8, 0.85, 0.9]) {
      for (const maxReturn5 of [999]) {
        for (const holdDays of [20]) {
          rows.push({
            id: `target_upside_${minTargetFullRatio}_nyh${minNearYearHigh}_h${holdDays}`,
            minTradeValue: 40e6,
            minScore: 45,
            minTargetFullRatio,
            minNearYearHigh,
...
async function main() {
  const identityInput = {
    strategyId: 'stock_target_upside_global_v2',
