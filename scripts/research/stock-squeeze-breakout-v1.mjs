...
const OUTPUT = new URL('../../data/research/stock-squeeze-breakout-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_SQUEEZE_BREAKOUT_V1.md', import.meta.url);
...
function pass(row, config) {
  ...
  if ((row.bollingerBandwidthPct || 999) > config.maxBandwidth) return false;
  if ((row.volatilityCompression5To20 || 999) > config.maxCompression) return false;
  if ((row.bollingerPercentB || 0) < config.minPercentB) return false;
  if ((row.donchian20Position || 0) < config.minDonchian) return false;
  if ((row.volumeRatio1To20 || 0) < config.minVolumeRatio) return false;
  ...
  if (config.requireBreakout && !row.bollingerUpperBreakout && !row.donchian20Breakout) return false;
  if (config.requireMa20Rising && !row.ma20Rising) return false;
  return true;
}

function rank(row, mode) {
  const breakout = (row.bollingerUpperBreakout ? 20 : 0) + (row.donchian20Breakout ? 15 : 0);
  const power = (row.signalScore || 0) + (row.volumeRatio1To20 || 0) * 12 + (row.return20Pct || 0);
  const risk = (row.atr14Pct || 0) * 2 + Math.max(0, row.gapUpPct || 0) + Math.max(0, row.distanceToMa20Pct || 0);
  if (mode === 'compressed_power') return breakout + power - risk - (row.bollingerBandwidthPct || 0);
  return breakout + power - risk;
}
...
function configs() {
  const rows = [];
  for (const maxBandwidth of [8, 12, 16]) {
    for (const minPercentB of [0.65, 0.8]) {
      for (const minVolumeRatio of [1.2, 1.6, 2]) {
        for (const holdDays of [5, 8, 12]) {
          rows.push({
            id: `squeeze_bw${maxBandwidth}_pb${minPercentB}_vol${minVolumeRatio}_h${holdDays}`,
            minTradeValue: 40e6,
            minScore: 58,
            maxBandwidth,
            maxCompression: 0.9,
            ...
            maxAtr: 6.5,
            minMarketMove: -2,
            requireBreakout: minPercentB >= 0.8,
            requireMa20Rising: true,
            holdDays,
            stopLossPct: holdDays <= 5 ? 4.5 : 6,
            takeProfitPct: holdDays <= 5 ? 8 : 14,
            trailTriggerPct: holdDays <= 5 ? 6 : 10,
            trailGivebackPct: 4,
            rankMode: 'compressed_power',
            maxEntriesPerDay: 5,
            maxOpen: 10,
            positionPct: 10,
            accountRiskPct: 1,
...
async function main() {
  const identityInput = {
    strategyId: 'stock_squeeze_breakout_v1',
    dataSources: ['tw_backtest_candidate_features', 'bollinger_features', 'daily_ohlcv'],
    setupRules: ['純個股低波動壓縮', '布林帶位置轉強', '量能放大', 'MA20 上彎'],
    triggerRules: ['布林上緣或 20 日高位轉強後進場'],
