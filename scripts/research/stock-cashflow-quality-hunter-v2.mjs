...
const OUTPUT = new URL('../../data/research/stock-cashflow-quality-hunter-v2.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_CASHFLOW_QUALITY_HUNTER_V2.md', import.meta.url);
const STRATEGY_ID = 'stock_cashflow_quality_hunter_v2';
...
function configurations() {
  return setups.flatMap(setup => [20, 40].flatMap(holdingDays => [10, 15].flatMap(topN => [8, 12].flatMap(stopDistancePct => ['positive_reaction', 'breakout_10d', 'ma20_restrengthen'].flatMap(trigger => ['risk_on'].map(marketMode => ({
