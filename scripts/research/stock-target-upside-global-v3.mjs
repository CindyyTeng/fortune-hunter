...
const OUTPUT = new URL('../../data/research/stock-target-upside-global-v3.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TARGET_UPSIDE_GLOBAL_V3.md', import.meta.url);
...
function pass(row, config) {
  ...
  if ((row.targetFull || 0) / row.entryPrice < config.minTargetFullRatio) return false;
  ...
  if ((row.globalCompositePct || 0) <= 0) return false;
...
function rank(row, mode) {
  const upside = ((row.targetFull || row.entryPrice) / row.entryPrice - 1) * 100;
  const power = (row.signalScore || 0) + upside * 1.5 + (row.nearYearHigh || 0) * 20 + (row.globalCompositePct || 0);
  const risk = (row.atr14Pct || 0) * 2 + Math.max(0, row.gapUpPct || 0) + Math.max(0, row.distanceToMa20Pct || 0);
  if (mode === 'target_global') return power - risk;
  return power + (row.themeMovePct || 0) * 2 - risk;
}
...
function configs() {
  const rows = [];
  for (const minTargetFullRatio of [1.15, 1.18]) {
    for (const minNearYearHigh of [0.8, 0.9]) {
      for (const holdDays of [15, 20]) {
        for (const stopLossPct of [10, 12]) {
          for (const positionPct of [4, 5]) {
            for (const maxOpen of [12, 18]) {
              for (const maxEntriesPerDay of [5]) {
                for (const monthlyBrakePct of [-6]) {
                  rows.push({
                    id: `target_upside_v3_${minTargetFullRatio}_nyh${minNearYearHigh}_h${holdDays}_s${stopLossPct}_p${positionPct}_o${maxOpen}`,
                    minTradeValue: 40e6,
                    minScore: 45,
                    minTargetFullRatio,
                    minNearYearHigh,
...
function score(row) {
  if (row.trades < 35 || row.averageMonthlyReturnPct <= 0 || row.profitFactor < 1.05) return -Infinity;
  return row.averageMonthlyReturnPct * 6 + row.profitFactor * 2.5 + row.maximumDrawdownPct * 0.18 + Math.min(row.trades, 260) / 130;
}
...
async function main() {
  const identityInput = {
    strategyId: 'stock_target_upside_global_v3',
    dataSources: ['tw_backtest_candidate_features', 'target_upside_features', 'global_composite_features'],
    setupRules: ['純個股目標空間大於 15%/18%/22%', '近年高相對位置不弱', '全球環境為正'],
    triggerRules: ['候選股隔日進場，固定 20 日驗證'],
    invalidationRules: ['固定停損', '移動停利回吐', '帳戶回撤冷卻'],
    exitRules: ['短持有', '停利', '移動停利', '停損'],
    riskRules: { noEtf: true, trainMonths: 60, validationMonths: 18, stepMonths: 18, riskSearch: true },
  ...
  await fs.writeFile(OUTPUT, `${JSON.stringify(output, null, 2)}\n`, 'utf8');
  await fs.writeFile(REPORT, `# 純個股目標空間全球順風 v3\n\n- 驗證期：${output.validation.startMonth}~${output.validation.endMonth}\n- 月均報酬：${metrics.averageMonthlyReturnPct}%\n- 年化報酬：${metrics.annualizedReturnPct}%\n- 最大回撤：${metrics.maximumDrawdownPct}%\n- 交易筆數：${metrics.trades}\n- Profit Factor：${metrics.profitFactor}\n- 勝率：${metrics.winRatePct}%\n- 0050 月均：${benchmark.averageMonthlyReturnPct}%\n- 是否達月均 5%：${passed}\n- 結論：${output.conclusion}\n\n此策略只交易個股，不以 ETF/0050 為主；核心仍是 targetFull 目標空間、近高位置與全球順風，但 v3 改測停損、持有天數、部位、持倉上限與月度煞車，目標是保留 v2 交易數並降低最大回撤。\n`, 'utf8');
  await appendExperiment({ ...identityInput, parameters: { configsTested: allConfigs.length }, trainPeriod: { months: 60 }, validationPeriod: { months: 18, stepMonths: 18 }, costModel: { roundTripPct: COST_PCT }, executionModel: { entry: 'candidate_next_entry', exit: 'fixed 20-day hold with wide protective stop', settlement: 'T+2 cash delay' }, metrics, resultStatus: passed ? 'passed' : 'failed', failureReason: passed ? null : output.conclusion, passedMinimum: passed, passedHighProfit: false, allowRetest: false, notes: '純個股目標空間全球順風第二版，不以 ETF 或 0050 為交易主體。' });
