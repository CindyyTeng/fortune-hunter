...
const OUTPUT = new URL('../../data/research/stock-top5-subset-portfolio-v4.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_TOP5_SUBSET_PORTFOLIO_V4.md', import.meta.url);
...
async function main() {
  ...
  const output = {
    ...
    strategyId: 'stock_top5_subset_portfolio_v4',
    source: 'stock-fixed-top5-oos-v13 trade details',
    selected: { filter: selected.filter, exposureScale: 1.4, train: summarize(trainTrades.filter(row => pass(row, selected.filter)), 1.4) },
    validation,
