...
const OUTPUT = new URL('../../data/research/stock-near-high-kd-fixed-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_NEAR_HIGH_KD_FIXED_V1.md', import.meta.url);
...
function configs() {
  const rows = [];
  for (const maxStochastic of [65, 70, 75]) {
    for (const minNearYearHigh of [0.88, 0.9, 0.92]) {
      for (const maxReturn5 of [3, 6, 10]) {
