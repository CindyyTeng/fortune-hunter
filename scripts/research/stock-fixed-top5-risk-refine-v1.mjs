...
const OUTPUT = new URL('../../data/research/stock-fixed-top5-risk-refine-v1.json', import.meta.url);
const REPORT = new URL('../../docs/STOCK_FIXED_TOP5_RISK_REFINE_V1.md', import.meta.url);
const INITIAL_CAPITAL = 1_000_000;
const STRATEGY_ID = 'stock_fixed_top5_risk_refine_v1';
...
function folds(allMonths, trainMonths = 60, validationMonths = 18, stepMonths = 12) {
  const rows = [];
...
function exitPriceFromForward(trade, config) {
  ...
  for (let index = 0; index <= maxIndex; index += 1) {
    ...
    const trailStop = highWater >= trade.entryPrice * (1 + config.trailStartPct / 100)
      ? Math.max(trade.entryPrice, highWater * (1 - config.trailGivebackPct / 100))
      : null;
    const stop = Math.max(hardStop, trailStop || 0);
    if ((bar.low ?? bar.price) <= stop) {
      return { date: bar.date, price: Math.min(bar.open ?? stop, stop), reason: '停損或移動停利' };
    }
    if (config.takeProfitPct && (bar.high ?? bar.price) >= trade.entryPrice * (1 + config.takeProfitPct / 100)) {
      const target = trade.entryPrice * (1 + config.takeProfitPct / 100);
      return { date: bar.date, price: Math.max(bar.open ?? target, target), reason: '停利' };
    }
  }
  return { date: exit.date, price: exit.price, reason: '持有到期' };
}
...
function score(trade, mode) {
  if (mode === 'fixedScore') return trade.signalScore || 0;
  if (mode === 'quality') return (trade.signalScore || 0) + (trade.nearYearHigh || 0) * 20 + (trade.ma20Slope5Pct || 0) * 4 - (trade.upperWickRatio || 0) * 12;
  return (trade.signalScore || 0) + (trade.return20Pct || 0) * 0.6 - (trade.atr14Pct || 0) * 4 - Math.max(0, trade.distanceToMa20Pct || 0);
}
...
function prepare(trades, config, startMonth, endMonth) {
  ...
  for (const trade of trades) {
    ...
    const exit = exitPriceFromForward(trade, config);
    if (!exit) continue;
    ...
    rows.push({
      ...
      netReturnPct: netReturn(trade.entryPrice, exit.price),
...
function simulate(trades, config, startMonth, endMonth) {
  ...
  const monthRows = months(startMonth, endMonth).map(month => {
...
function configs() {
  ...
  for (const positionPct of [5, 6, 8]) {
    for (const maxOpenPositions of [8, 10, 12]) {
      for (const maxEntriesPerDay of [2, 3, 5]) {
        for (const rankMode of ['fixedScore', 'quality', 'riskAdjusted']) {
...
async function main() {
  const identityInput = {
    ...
    dataSources: ['OHLCV 候選池'],
    setupRules: ['從 fixed top5 正期望策略延伸', '純個股', '少量風控與排序調整'],
    triggerRules: ['訊號日後隔日進場'],
    invalidationRules: ['月虧損熔斷', '連虧冷卻', '停損與移動停利'],
    exitRules: ['15 日持有、停損、移動停利'],
    riskRules: { tPlusTwo: true, positionPct: [5, 6, 8], monthlyBrakePct: -6 },
    parameters: { trainMonths: 60, validationMonths: 18, configs: configs().length },
    costModel: '手續費、交易稅、滑價、T+2',
    executionModel: '純個股候選池，使用 entryDate 後 forward bar 模擬出場'
  };
  ...
  const allFolds = folds(allMonths, 60, 18, 12);
  ...
  for (const fold of allFolds) {
    ...
    for (const config of allConfigs) {
      ...
      if (train.trades < 50) continue;
      const objective = train.averageMonthlyReturnPct / Math.max(1, Math.abs(train.maximumDrawdownPct)) + Math.min(1, train.trades / 500) * 0.2;
